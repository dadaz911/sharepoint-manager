...
cands=[]
for p in dirs:
    if not re.match(r'^NIT-0+\d', p): continue
    num=p.split("-",1)[1].lstrip("0"); canon=f"NIT-{num}"
    if canon not in dirs: continue
...
for p,canon in sorted(cands):
    pf=sp_files(p); cf=sp_files(canon)
    if pf is None or cf is None:
        skipped.append((p,"no listable en SP")); continue
    bad=[f for f in pf if f not in cf or cf[f]!=pf[f]]
    if bad:
        skipped.append((p,f"NO redundante en SP: {bad}")); continue   # 2a verificación falla -> NO borrar
    if DRY:
        recycled+=1; continue
    ...
    for f in pf:
        rf=recycle_file(p, f)
        if rf.status_code not in (200,204): fail=f"archivo {f} -> HTTP {rf.status_code}"; break
    if fail:
        skipped.append((p,fail)); continue
    r=recycle(p)
    if r.status_code in (200,204): recycled+=1
    else: skipped.append((p,f"carpeta recycle HTTP {r.status_code}: {r.text[:80]}"))
