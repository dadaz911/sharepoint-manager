...
for d in os.listdir(BASE):
    ...
    if not m or not (m.group(2).lstrip("0") or "0").isdigit():
        skipped.append((d,"nombre no estándar")); continue
...
for n, folders_ in num2.items():
    ...
    if canon is None:
        skipped += [(nm,"sin canónica") for nm,_ in folders_]; continue
    cpath=os.path.join(BASE,canon)
    csizes={f:os.path.getsize(os.path.join(cpath,f)) for f in os.listdir(cpath) if os.path.isfile(os.path.join(cpath,f))}
    for nm,_ in folders_:
        if nm==canon: continue
        npath=os.path.join(BASE,nm); leftover=False
        for f in list(os.listdir(npath)):
            sp=os.path.join(npath,f)
            if not os.path.isfile(sp): leftover=True; continue
            sz=os.path.getsize(sp)
            if f in csizes:
                if csizes[f]==sz:
                    if not DRY: os.remove(sp)
                    removed+=1
                else:
                    skipped.append((f"{nm}/{f}","tamaño distinto en canónica")); leftover=True
            else:
                if not DRY: os.replace(sp, os.path.join(cpath,f)); csizes[f]=sz
                moved+=1
        # borrar carpeta si quedó vacía
        try:
            if not leftover and not os.listdir(npath):
                if not DRY: os.rmdir(npath)
                folders+=1
            elif leftover:
...
print(f"{'(DRY) ' if DRY else ''}archivos movidos: {moved} | archivos redundantes eliminados: {removed} | carpetas eliminadas: {folders} | saltadas: {len(skipped)}")
