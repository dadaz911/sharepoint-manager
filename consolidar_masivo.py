...
"""
import os, sys, time, json, re, threading, urllib.parse, collections
...
_lock=threading.Lock(); _tok={"v":None}; _pause={"t":0.0}
_done=set(); _stats=collections.Counter(); _errlog=None

def log(m): print(f"[{time.strftime('%H:%M:%S')}] {m}", flush=True)
def load_token():
    with _lock: _tok["v"]=open(TOKEN_FILE).read().strip()
def norm(num): return (num.strip().lstrip("0") or "0")
def enc(p): return urllib.parse.quote(p, safe="/")

def _req(s, method, url, **kw):
    for a in range(MAX_RETRIES):
        with _lock: w=_pause["t"]-time.time()
        if w>0: time.sleep(min(w,5))
        h=kw.pop("headers",{}); h["Authorization"]=f"Bearer {_tok['v']}"
        try: r=s.request(method,url,headers=h,timeout=60,**kw)
        except requests.RequestException:
            time.sleep(min(2**a,30)); kw["headers"]=h; continue
        if r.status_code in (429,503):
            ra=r.headers.get("Retry-After"); secs=int(ra) if ra and ra.isdigit() else 30
            with _lock: _pause["t"]=max(_pause["t"], time.time()+secs+a*5)
            kw["headers"]=h; continue
        if r.status_code==401:
            load_token(); time.sleep(2); kw["headers"]=h; continue
        return r
    return None

def build_plan():
    num2=collections.defaultdict(list); skipped=[]
    for name in os.listdir(SRC):
        if not os.path.isdir(os.path.join(SRC,name)): continue
    ...
    for n,folders in num2.items():
        ...
        if not reals:
            skipped += [f"{nm} (sin canónica)" for nm,_ in folders]; continue
...
def process(s, noncanon, canon, files):
    ...
    if r is None or r.status_code not in (200,204):
        ...
        with _lock: _stats["err"]+=1
        return False
    with _lock: _stats["folders"]+=1; _done.add(noncanon)
    return True

def save_progress():
    with _lock: data=sorted(_done)
    tmp=PROGRESS+".tmp"; open(tmp,"w").write(json.dumps({"done":data})); os.replace(tmp,PROGRESS)

def main():
    load_token()
    if os.path.exists(PROGRESS):
        try: _done.update(json.load(open(PROGRESS)).get("done",[]))
        except Exception: pass
    plan,skipped=build_plan()
    if ONLY: plan=[x for x in plan if x[0]==ONLY]; log(f"SPM_ONLY={ONLY} -> {len(plan)} carpeta(s)")
    ...
    if DRYRUN:
        for nc,cn,fs in pending[:10]: log(f"  DRYRUN  {nc}  ->  {cn}   ({len(fs)} archivos)")
        log(f"(dry-run; saltadas ej.: {skipped[:3]})"); return
    global _errlog; _errlog=open(os.path.join(SRC,".consolidate_errors.log"),"a")
    if not pending: log("nada pendiente"); return
    tl=threading.local()
    def work(item):
        s=getattr(tl,"s",None)
        if s is None: s=requests.Session(); tl.s=s
        process(s,*item)
        with _lock: n=_stats["folders"]
        if n and n%200==0: save_progress()
    with ThreadPoolExecutor(max_workers=THREADS) as ex:
        list(ex.map(work,pending))
    save_progress()
    log(f"FIN. carpetas consolidadas={_stats['folders']} | archivos movidos={_stats['moved']} | errores={_stats['err']}")
